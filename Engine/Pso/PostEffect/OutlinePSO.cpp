...
OutlinePSO::OutlinePSO(IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler, const std::string& VS_fileName, const std::string& PS_fileName) {
	// 初期化
	Init(dxcUtils, dxcCompiler, includeHandler, VS_fileName, PS_fileName);
}

void OutlinePSO::Init(IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler, const std::string& VS_fileName, const std::string& PS_fileName) {
	// 基底クラスの初期化
...
#pragma region sampler
	psoData_.staticSamplers_.resize(2);
	...
	psoData_.staticSamplers_[1].Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;
	psoData_.staticSamplers_[1].AddressU = D3D12_TEXTURE_ADDRESS_MODE_WRAP;
	psoData_.staticSamplers_[1].AddressV = D3D12_TEXTURE_ADDRESS_MODE_WRAP;
	psoData_.staticSamplers_[1].AddressW = D3D12_TEXTURE_ADDRESS_MODE_WRAP;
	psoData_.staticSamplers_[1].ComparisonFunc = D3D12_COMPARISON_FUNC_NEVER;
	psoData_.staticSamplers_[1].MaxLOD = D3D12_FLOAT32_MAX;
	psoData_.staticSamplers_[1].ShaderRegister = 1;
	psoData_.staticSamplers_[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
