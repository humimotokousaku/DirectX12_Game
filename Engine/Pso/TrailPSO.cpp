...
void TrailPSO::Init(IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler, const std::string& VS_fileName, const std::string& PS_fileName) {
	// 基底クラスの初期化
	IPSO::Init(dxcUtils, dxcCompiler, includeHandler, VS_fileName, PS_fileName);
}

void TrailPSO::CreateRootSignature() {
	HRESULT hr;
...
#pragma region rootParameter
	psoData_.rootParameters_.resize(3);
#pragma region VSに送るデータ
	...
	psoData_.rootParameters_[2].Descriptor.ShaderRegister = 1;
#pragma endregion
